...
def add_paragraph_with_links(text_parts, bold=False, italic=False, size=10):
...
# Set document margins - more compact
...
name_run = header_p.add_run(f"\n{PERSONAL_INFO['name']}")
name_run.bold = True
name_run.font.size = Pt(16)  # Slightly smaller
name_run.font.color.rgb = RGBColor(0x1f, 0x4e, 0x79)

title_run = header_p.add_run(f"\n\n{PERSONAL_INFO['title']}")
title_run.font.size = Pt(12)  # Smaller
...
contact_p.paragraph_format.space_after = Pt(12)  # Reduced from 16
contact_p.paragraph_format.space_before = Pt(6)   # Reduced from 8

# Add contact info with hyperlinks and separators - more compact layout
contact_parts = [
...
# Summary - simplified without background
add_section_header("Summary")
summary_p = add_paragraph(SUMMARY, size=10.5)
...
for i, project in enumerate(PROJECTS):
    ...
    # Project links - more compact format
    if "demo_urls" in project:
        link_parts = []
        for i, (name, url) in enumerate(project["demo_urls"].items()):
            if i > 0:
                link_parts.extend([" | "])  # Simple separator
            link_parts.extend([
                {"text": name, "url": url}
            ])
        ...
        add_paragraph_with_links([
            {"text": "Live", "url": project["demo_url"]},
            " | ",
...
for category, skills in SKILLS.items():
    p = doc.add_paragraph()
    p.paragraph_format.space_before = Pt(1)  # Reduced from 2
    p.paragraph_format.space_after = Pt(3)   # Reduced from 6
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.EXACTLY
    p.paragraph_format.line_spacing = Pt(12)  # Reduced from 14
    ...
    category_run.font.size = Pt(10)  # Reduced from 10.5
    ...
    skills_run.font.size = Pt(10)  # Reduced from 10.5
    skills_run.font.color.rgb = RGBColor(0x2c, 0x3e, 0x50)  # Dark color for skills

# Languages with simplified formatting
add_section_header("Languages")
for language in LANGUAGES:
    add_paragraph(language, size=10)
