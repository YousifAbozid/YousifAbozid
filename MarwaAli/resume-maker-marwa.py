...
def add_paragraph(text, bold=False, italic=False, size=10, indent=0):
    p = doc.add_paragraph()
    p.paragraph_format.space_before = Pt(0)
    p.paragraph_format.space_after = Pt(2)
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.EXACTLY
    p.paragraph_format.line_spacing = Pt(12)  # Reduced line spacing
...
# Certifications with compact formatting
add_section_header("Certifications")

# Display certifications in a more compact way - 2 per line
cert_pairs = []
for i in range(0, len(CERTIFICATIONS), 2):
    if i + 1 < len(CERTIFICATIONS):
        cert_pairs.append([CERTIFICATIONS[i], CERTIFICATIONS[i + 1]])
    else:
        cert_pairs.append([CERTIFICATIONS[i]])
...
for pair in cert_pairs:
    ...
    cert_run = p.add_run(pair[0])
    cert_run.font.size = Pt(10)
    cert_run.font.color.rgb = RGBColor(0x2c, 0x3e, 0x50)
    
    # Second certification (if exists)
    if len(pair) > 1:
        separator_run = p.add_run("     • ")
        separator_run.font.color.rgb = RGBColor(0x1f, 0x4e, 0x79)
        separator_run.font.size = Pt(10)
        
        cert_run2 = p.add_run(pair[1])
        cert_run2.font.size = Pt(10)
        cert_run2.font.color.rgb = RGBColor(0x2c, 0x3e, 0x50)
